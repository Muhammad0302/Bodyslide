...
const GenericThread = (props) => {
  ...
  const { singleSpa, isTrending } = location.state;
  const {
    setCommentOfType
  } = useContext(Context);
          ...
          >
            <MDBCol md={12} >
              ...
              <MDBRow style={{ padding: "1.5% 1%" }}>
                ...
                <MDBRow>
                  <MDBCol md={12}>
                    <p style={{ color: "#322d2d", fontSize: "20px" }}>
                      {Parser(singleSpa.description)}
                    </p>
                  </MDBCol>
                </MDBRow>
                <MDBRow>
                  <MDBCol md={6}>
                  <span className="daytime"> <TimeSinceCreation createdAt={singleSpa?.createdAt} /></span> 
                    &nbsp;&nbsp;&nbsp;
                    {singleSpa.userId == localStorage.getItem("userID") ||
                      localStorage.getItem("userID") == 10 ? (
                      <>
